...
|随同徽标键按下|功能|
|:-|:-|
|**`?`<br>`F1`<br>`F4`<br>`F5`**|**显示 Power Keys 的运行状态<br>打开 Power Keys 的帮助页面<br>退出 Power Keys<br>重启 Power Keys 以解决热键失灵问题或者退出游戏模式**
|**`G`<br>`F5`**|**进入游戏模式以禁用可能会影响您游戏体验的快捷键和徽标键<br>重启 Power Keys 以退出游戏模式**
|**`+` `-`**|**增大 / 减小系统音量**
...
|**`Tab`**|**全选**|**`Ctrl` + `A`**
|**`Caps Lock`**<br>**`Backspace`**|**回格<br>删除**|**`Backspace`<br>`Delete`**
|**`"`**<br>**`Enter`**|**回车<br>换行符**|**`Enter`<br>`Shift` + `Enter`**
|**`Shift`<br>`[` `]`**|**空格<br>向左 / 向右缩进**|**`Space`<br>`Shift` + `Tab` `Tab`**
|**`3` `8`**|**`#` / `*` （适用于 Markdown）**|**`Shift` + `3` `8`**
...
|**`(` `)`**|**向左 / 右挪动页面**|**`Shift` + `上滚轮` `下滚轮`**
|**`H` `G`**|**进入 / 退出右键菜单**|**`Shift` + `F10` `Esc`**
|**`1`**|**保存文档**|**`Ctrl` + `S`**
|**`Esc`**|**关闭窗口**|**`Alt` + `F4`**
...
如果您需要临时按住空格键以实现某些操作，请在按住空格键的同时按住 `Shift`；

如果您需要频繁地按下或按住空格键，那您肯定是在打游戏，请按下 `Win + G` 进入游戏模式。如需退出游戏模式，请按 `Win + F5`。
...
#### 最后，Power Keys 还能让您的笔记本电脑也拥有数字小键盘，助力您更加高效准确地输入数据。

<br>

**我们建议您在使用本功能前掌握数字小键盘的打字指法。掌握后，您只需要将中指放在 `J` 键上就可以尽情盲打数字了！**

<br>

|随同 `1` 按下|数字
|:-|:-
|**`Y` `U` `I`<br>`H` `J` `K`<br>`N` `M` `,`**|**`7` `8` `9`<br>`4` `5` `6`<br>`1` `2` `3`**
|**`Space`**|**`0`**
|**右侧 `Alt`**|**`.`**
|**`.`<br>右侧 `Ctrl`<br>`Enter`**|**`Enter`**
|**`BackSpace`**|**`BackSpace`**
|**`L` 或 `O`**|**`+`**
|**`9` `8` `7`**|**`-` `*` `/`**
|**`+` `-` `*` `/`**|**`+` `-` `*` `/`**
