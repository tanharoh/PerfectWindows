...
**更重要的是，Power Keys 还可以让您码字的过程更加流畅，让您的右手无需在主键区、方向键区和导航键区来回移动。**
> 即将到来。

|随同 `空格` 键按下|功能|模拟的快捷键
|:-:|:-|:-
|**`F` `J` `R` `M`**|**向左 / 右 / 上 / 下移动光标**|**`←` `→` `↑` `↓`**
|**`A`<br>`;`**|**将光标放在行首<br>将光标放在行末**|**`Home`<br>`End`**
|**`Q`<br>`/`**|**将光标放在文首<br>将光标放在文末**|**`Ctrl + Home`<br>`Ctrl + End`**
|**`D`<br>`K`**|**向左选定文本<br>向右选定文本**|**`Shift + 向左键`<br>`Shift + 向右键`**
|**`E`<br>`,`**|**向上选定文本<br>向下选定文本**|**`Shift + 向上键`<br>`Shift + 向下键`**
|**`S`<br>`L`**|**向左选定文本直到行首<br>向右选定文本直到行末**|**`Shift + Home`<br>`Shift + End`**
|**`W`<br>`.`**|**向上选定文本直到文首<br>向下选定文本直到文末**|**`Ctrl + Shift + Home`<br>`Ctrl + Shift + End`**
|**`B`<br>`I`<br>`U`**|**加粗文本<br>倾斜文本<br>为文本加下划线**|**`Ctrl + B`<br>`Ctrl + I`<br>`Ctrl + U`**
|**`O`<br>`P`**|**减小字号<br>增大字号**|**`Ctrl + Shift + ,`<br>`Ctrl + Shift + .`**
|**`T`<br>`N`**|**向上翻页<br>向下翻页**|**`Page Up`<br>`Page Down`**
|**`G`<br>`H`**|**向左挪动页面<br>向下翻页**|**`Page Up`<br>`Page Down`**
